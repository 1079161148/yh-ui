  ...
  <div :class="[ns.b(), ns.is('disabled', disabled), ns.is('multiple', multiple)]" ref="selectRef">
    <div :class="[ns.e('trigger'), ns.is('active', visible)]" @click="handleTriggerClick" @mouseenter="showClear = true"
      ...
      <div :class="ns.e('tags')">
        <template v-if="multiple && Array.isArray(modelValue)">
          ...
          <div v-for="(item, index) in (collapseTags ? selectedLabels.slice(0, maxCollapseTags) : selectedLabels)"
            :key="item.value" :class="ns.e('tag')">
          ...
          <!-- 折叠占位符 -->
          <div v-if="collapseTags && selectedLabels.length > maxCollapseTags" :class="[ns.e('tag'), ns.m('info')]">
            <span>+ {{ selectedLabels.length - maxCollapseTags }}</span>
          </div>
        </template>

        <input ref="inputRef" v-model="query" :placeholder="inputPlaceholder" :disabled="disabled"
          :readonly="!filterable"
          :class="[ns.e('inner'), ns.is('invisible', !multiple && hasValue && !visible && !query)]" autocomplete="off"
          @input="handleInput" @focus="handleInputFocus" />

        <span :class="ns.e('suffix')">
          <span v-if="showClear && clearable && hasValue" :class="ns.e('clear')" @click.stop="handleClear">
            <svg viewBox="0 0 1024 1024" width="1em" height="1em">
              <path fill="currentColor"
                d="M512 64a448 448 0 1 1 0 896 448 448 0 0 1 0-896zm0 393.664L407.936 353.6a38.4 38.4 0 1 0-54.336 54.336L457.664 512 353.6 616.064a38.4 38.4 0 1 0 54.336 54.336L512 566.336 616.064 670.4a38.4 38.4 0 1 0 54.336-54.336L566.336 512 670.4 407.936a38.4 38.4 0 1 0-54.336-54.336L512 457.664z" />
            </svg>
    ...
    <Teleport to="body" :disabled="!teleported">
      <Transition name="yh-zoom-in-top">
        <div v-if="visible" ref="popperRef" :class="[ns.e('popper'), popperClass]" :style="dropdownStyle">
          <div :class="ns.e('dropdown')">
            <div ref="virtualWrapperRef" :class="ns.e('options-wrapper')"
              :style="{ maxHeight: heightNumber + 'px', overflow: 'auto' }" @scroll="handleScroll">

              <!-- 列表区域 -->
              <div v-if="flatData.length > 0" :style="listContainerStyle">
                <div :style="listShiftStyle">
                  <div v-for="node in renderedNodes" :key="node.key"
                    :class="[ns.e('node'), ns.is('selected', node.checked), ns.is('disabled', node.disabled)]"
                    :style="{ paddingLeft: `${node.level * indent + 8}px`, height: itemSize + 'px' }"
                    @click.stop="handleNodeClick(node, $event)">
                    <div :class="ns.e('node-content')">
                      <!-- 展开图标 -->
                      <span :class="[ns.e('expand-icon'), ns.is('leaf', node.isLeaf), ns.is('expanded', node.expanded)]"
                        ...
                        <svg v-else :class="ns.is('loading')" viewBox="0 0 1024 1024" width="1em" height="1em">
                          <path fill="currentColor"
                      ...
                      <span v-if="showCheckbox"
                        ...
                        <span :class="ns.e('checkbox-inner')">
                          <svg v-if="node.checked && !node.indeterminate" viewBox="0 0 1024 1024" width="1em"
                            height="1em">
                            <path fill="currentColor"
                              d="M406.656 706.944L195.84 496.256a32 32 0 1 0-45.248 45.248l256 256 512-512a32 32 0 0 0-45.248-45.248L406.592 706.944z" />
                          </svg>
                          <svg v-else-if="node.indeterminate" viewBox="0 0 1024 1024" width="1em" height="1em">
...
const { flatData, nodeMap, mapVersion, checkNode, toggleExpand, filter, getNode } = useTree(props, emit)
...
const renderedNodes = computed(() => {
  ...
  const end = Math.min(flatData.value.length, start + Math.ceil(heightNumber.value / props.itemSize) + 10)
...
const handleOutsideClick = (e: MouseEvent) => {
  ...
  if (visible.value && selectRef.value && !selectRef.value.contains(target) &&
...
const handleClear = () => {
  ...
  emit('clear')
  nextTick(() => {
    nodeMap.value.forEach(n => { n.checked = false; n.indeterminate = false })
    mapVersion.value++
  })
}
...
onMounted(() => { if (visible.value) updatePopper() })
onBeforeUnmount(() => {
  window.removeEventListener('click', handleOutsideClick, true)
  window.removeEventListener('scroll', updatePopper, true)
  window.removeEventListener('resize', updatePopper)
