...
// 监听数据变化以自动展开（避免在 computed 中产生副作用）
watch(
  () => props.data,
  (newData) => {
  ...
  <div ref="tableRef" :class="tableClasses" :style="tableStyle">
    <div :class="innerWrapperClasses">
        ...
        <table :class="ns.e('header')" :style="{ tableLayout }">
          <colgroup>
            <col
              ...
              :style="{ width: formatSize(selectionConfig.columnWidth || 50) }"
            />
            <col
              v-if="expandConfig"
              :style="{ width: formatSize(expandConfig.columnWidth || 50) }"
            />
                  ...
                  <div :class="ns.e('cell-content')">
                    ...
                    <component
                      v-if="
                        ...
                        typeof cell.column.headerPrefixIcon !== 'string'
                      "
                    ...
                    <component
                      v-if="
                        ...
                        typeof cell.column.headerSuffixIcon !== 'string'
                      "
                    ...
                    <span v-if="cell.column.sortable" :class="ns.e('sort-icon')">
                      <span
                        :class="[
                          'sort-caret',
                          'ascending',
                          getSortOrder(cell.column) === 'asc' ? 'is-active' : ''
                        ]"
                      />
                      <span
                        :class="[
                          'sort-caret',
                          'descending',
                          getSortOrder(cell.column) === 'desc' ? 'is-active' : ''
                        ]"
                      />
                    </span>
                  </div>
                  ...
                  <span v-if="column.sortable" :class="ns.e('sort-icon')">
                    <span
                      :class="[
                        'sort-caret',
                        'ascending',
                        getSortOrder(column) === 'asc' ? 'is-active' : ''
                      ]"
                    />
                    <span
                      :class="[
                        'sort-caret',
                        'descending',
                        getSortOrder(column) === 'desc' ? 'is-active' : ''
                      ]"
                    />
                  </span>
                </div>
          ...
          <tbody>
            <template v-for="(row, rowIndex) in renderData" :key="getRowKeyFn(row)">
              ...
              >
                ...
                <td
                  ...
                  :class="[
                    ns.e('cell'),
                    ns.e('selection-cell'),
                    isAnyColumnFixedLeft ? 'is-fixed-left' : ''
                  ]"
                ...
                >
                  <td
                    ...
                    :colspan="
                      calculateSpan(row, column, rowIndex, columnIndex, spanMethod).colspan ||
                      undefined
                    "
                    :rowspan="
                      calculateSpan(row, column, rowIndex, columnIndex, spanMethod).rowspan ||
                      undefined
                    "
                    :data-row-key="getRowKeyFn(row)"
                    :data-prop="column.prop"
                    @click="
                      handleCellClick(row, column, $event.currentTarget as HTMLElement, $event)
                    "
                    @dblclick="
                      handleCellDblclick(row, column, $event.currentTarget as HTMLElement, $event)
                    "
                  >
                    <div :class="ns.e('cell-content')">
                      <!-- 树形缩进 -->
                      <span
                        ...
                        :style="{
                          paddingLeft: `${(((row as Record<string, unknown>)._level as number) || 0) * (treeConfig.indent || 16)}px`
                        }"
                      >
                        <span
                          v-if="(row as Record<string, unknown>)._hasChildren"
                          :class="[
                            ns.e('tree-icon'),
                      ...
                      <template v-else>
                        <yh-tooltip
                          ...
                          :effect="
                            typeof column.showOverflowTooltip === 'object'
                              ? column.showOverflowTooltip.effect || tooltipEffect
                              : tooltipEffect
                          "
                          :placement="
                            typeof column.showOverflowTooltip === 'object'
                              ? column.showOverflowTooltip.placement || 'top'
                              : 'top'
                          "
                        >
              ...
              >
                <td
                  :colspan="
                    visibleColumns.length + (selectionConfig ? 1 : 0) + (showIndex ? 1 : 0) + 1
                  "
                >
                  <slot name="expand" :row="row" :row-index="rowIndex">
                    <component
                      v-if="expandConfig.render"
                      :is="expandConfig.render({ row, rowIndex })"
                    />
                  </slot>
                </td>
              </tr>
                    ...
                    >
                      {{
                        summaryValues.length > 0
                          ? summaryValues[columnIndex]
                          : columnIndex === 0
                            ? summaryConfig.text || t('table.sumText')
                            : ''
                      }}
                    </slot>
                  </div>
      ...
      <div
        ...
        :class="ns.e('loading')"
        :style="
          typeof loading === 'object' && loading.background
    ...
    <div
      v-if="pagination"
      :class="[
        ns.e('pagination-wrapper'),
        typeof pagination === 'object' && pagination.align ? ns.is('align-' + pagination.align) : ''
      ]"
    >
      <YhPagination
            ...
            emit('page-change', {
              ...
              pageSize:
                (typeof pagination === 'object'
                  ? (pagination as TablePaginationConfig).pageSize
                  : 10) || 10
            })
        "
        @update:page-size="
          (val: number) =>
            emit('page-change', {
              currentPage:
                (typeof pagination === 'object'
                  ? (pagination as TablePaginationConfig).currentPage
                  : 1) || 1,
              pageSize: val
            })
        "
