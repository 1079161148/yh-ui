...
export default defineBuildConfig([
  // ── First Config: existing mkdist-based compilation ──
  {
    entries: [
      {
        builder: 'mkdist',
      ...
      // 生成完全自包含 of dayjs-plugins.mjs / .cjs，消费方零配置。
      {
        builder: 'rollup',
    ...
    hooks: {
      ...
      async 'mkdist:entry:options'(_ctx, _entry, options) {
        const customSassLoader = async (input: {
          ...
          const compileString =
            sass.compileString ??
            (sass as unknown as { default: typeof sass }).default.compileString
  ...
  // ── Second Config: rollup-based Node/SSR entry compiling Vue SFCs ──
  {
    entries: [
      {
        input: './src/index',
    ...
    hooks: {
      'build:before'(ctx) {
        // Filter out dayjs from externals to force Rollup to bundle/inline it
        ctx.options.externals = ctx.options.externals.filter(
          (ext) => !(typeof ext === 'string' && (ext === 'dayjs' || ext.startsWith('dayjs/')))
        )
      },
      'rollup:options'(ctx, options) {
        ...
        // vue() plugin must run AFTER ignore-styles so ignore-styles load hook intercepts style queries first
        options.plugins.unshift(vue())
        options.plugins.unshift({
          name: 'ignore-styles',
          resolveId(source) {
            if (
              source.includes('?vue&type=style') ||
