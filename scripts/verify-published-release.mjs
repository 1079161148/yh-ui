...
import { findProtocolLeaks, formatProtocolLeaks } from './packed-manifest-utils.mjs'
...
function getPublishedManifest(metadata, pkg) {
  if (metadata.name === pkg.manifest.name && metadata.version === pkg.manifest.version) {
    return metadata
  }

  const manifest = metadata.versions?.[pkg.manifest.version]

  if (!manifest) {
    throw new Error(
      `${pkg.manifest.name} registry metadata is missing versions.${pkg.manifest.version}`
    )
  }
...
async function assertPackageVersionVisible(pkg) {
  ...
  const publishedManifest = getPublishedManifest(metadata, pkg)
  const protocolLeaks = findProtocolLeaks(publishedManifest)

  if (protocolLeaks.length > 0) {
    throw new Error(
      `[release-published] ${pkg.manifest.name}@${pkg.manifest.version} registry manifest still contains workspace/catalog protocols:\n${formatProtocolLeaks(protocolLeaks)}`
    )
  }
}
