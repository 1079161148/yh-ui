...
var xi = (a, _, r) =>
  _ in a ? ki(a, _, { enumerable: !0, configurable: !0, writable: !0, value: r }) : (a[_] = r)
var G = (a, _, r) => xi(a, typeof _ != 'symbol' ? _ + '' : _, r)
import {
  inject as re,
  ref as P,
  unref as N,
...
const wi = 'yh',
  ...
  oe = 2e3,
  ji = Symbol('zIndexContextKey'),
  Ti = Symbol('zIndexCounterKey'),
  $i = () => {
    if (typeof window < 'u') {
      const a = window
  ...
  Cy = (a) => {
    ...
    return {
      ...
      nextZIndex: () => {
        const e = N(a)
        return e !== void 0 ? e : r ? ++r.current : $i()
      }
    }
  }
function Ry(a, _, r = []) {
  const t = P(r),
    ...
    n = (u, l) => {
      ...
      const m = d
        .filter((f) => !!f)
        .sort((f, y) => String(f).localeCompare(String(y)))
        .join(',')
    ...
    i = R(() => {
      ...
      return (
        _.find(
          (d) =>
            [...d.specValueIds].sort((m, f) => String(m).localeCompare(String(f))).join(',') === l
        ) || null
      )
    })
  function o(u) {
    ...
    for (const d of u) {
      const m = l.length
      for (let f = 0; f < m; f++) l.push([...l[f], d])
    }
    return l
      .filter((d) => d.length > 0)
      .map((d) => [...d].sort((m, f) => String(m).localeCompare(String(f))))
  }
  return {
    selectedValueIds: t,
    isValueSelectable: n,
...
function Fy(a) {
  const { time: _, interval: r = 1e3, onFinish: t, onChange: s } = a,
    n = P(_),
    e = P(null),
    i = R(() => e.value !== null),
    o = () => {
...
const Gr = {
  ...
  qi = Symbol('configProviderContextKey'),
  Pi = () => {
    const a = re(qi, null),
      _ = R(() => {
        const s = N(a)
          ...
          var r = 1e3,
            ...
            b = {
              ...
              m: function D(L, v) {
                ...
                var Y = 12 * (v.year() - L.year()) + (v.month() - L.month()),
                  h = L.clone().add(Y, d),
                  x = v - h < 0,
                  S = L.clone().add(Y + (x ? -1 : 1), d)
              ...
              p: function (D) {
                return (
                  { M: d, y: f, w: l, d: u, D: y, h: o, m: i, s: e, ms: n, Q: m }[D] ||
                  String(D || '')
                    .toLowerCase()
                    .replace(/s$/, '')
                )
          ...
          var B = (function () {
              ...
              return (
                (L.parse = function (v) {
                  ;((this.$d = (function (Y) {
                    ...
                    if (typeof h == 'string' && !/Z$/i.test(h)) {
                      ...
                      if (S) {
                        var $ = S[2] - 1 || 0,
                          q = (S[7] || '0').substring(0, 3)
                        return x
                          ? new Date(
                              Date.UTC(S[1], $, S[3] || 1, S[4] || 0, S[5] || 0, S[6] || 0, q)
                            )
                          : new Date(S[1], $, S[3] || 1, S[4] || 0, S[5] || 0, S[6] || 0, q)
                      }
                    }
                    return new Date(h)
                  })(v)),
                  ...
                  var h = this,
                    ...
                    q = function (X, U) {
                      return A.w(
                        h
                          .toDate()
                          [X].apply(h.toDate('s'), (x ? [0, 0, 0, 0] : [23, 59, 59, 999]).slice(U)),
                        h
                      )
                    },
                    I = this.$W,
                ...
                (L.add = function (v, Y) {
                  ...
                  if (S === u) return $(1)
                  if (S === l) return $(7)
                  var q = ((h = {}), (h[i] = t), (h[o] = s), (h[e] = r), h)[S] || 1,
                    I = this.$d.getTime() + v * q
                  return A.w(I, this)
                }),
                ...
                (L.diff = function (v, Y, h) {
                  var x,
                    ...
                    $ = A.p(Y),
                    q = z(v),
                    I = (q.utcOffset() - this.utcOffset()) * t,
                    J = this - q,
                    K = function () {
                      return A.m(S, q)
                    }
...
const Es = /* @__PURE__ */ p(Ps),
  ...
  Fi = async (a) => {
    const _ = `../../../../node_modules/dayjs/locale/${a}.js`,
      r = Ci[_]
    if (r) return (await r(), !0)
    try {
      return (
        await import(
          /* @vite-ignore */
  ...
  Jy = (a, _) => {
    const r = Wr(a),
      t = [_.jan, _.feb, _.mar, _.apr, _.may, _.jun, _.jul, _.aug, _.sep, _.oct, _.nov, _.dec]
    try {
  ...
  Ny = (a) => {
    ...
    const s = (i) => {
      const o = i.split('.'),
        u = [r.value.yh, Gr.yh, Ai.yh]
      for (const l of u) {
        let d = l
        for (const m of o)
          if (d && typeof d == 'object') d = d[m]
          else {
            d = void 0
            break
          }
  ...
  Ii = Symbol('idInjectionKey'),
  Oy = (a) => {
    const _ = re(Ii, void 0),
      r = Di()
    return R(() => {
      const s = N(a)
      if (s) return s
  ...
  Ji = Symbol('FormContextKey'),
  Ni = Symbol('FormItemContextKey'),
  By = () => {
    const a = re(Ji, void 0),
      _ = re(Ni, void 0)
    return {
      form: a,
      formItem: _,
...
function Wy(a) {
  const _ = P(null),
    r = P(0),
    t = R(() => (Array.isArray(a.items) ? a.items : a.items.value)),
    s = R(() => N(a.itemHeight)),
    n = R(() => N(a.containerHeight)),
    e = R(() => N(a.overscan) ?? 3),
    i = R(() => t.value.length * s.value),
    o = R(() => {
    ...
    l = R(() => t.value.slice(o.value, u.value)),
    d = R(() => o.value * s.value)
  return {
    visibleItems: l,
    totalHeight: i,
...
const Gy = (a) => {
  const _ = P(!1)
  let r = { overflow: '', paddingRight: '' },
    t = { overflow: '', paddingRight: '' }
  const s = () => window.innerWidth - document.documentElement.clientWidth,
    n = () => {
      ...
      ;((o.style.overflow = 'hidden'),
        ...
        o.classList.add('yh-popup-parent--hidden'),
        (_.value = !0))
    },
    e = () => {
      if (!_.value) return
      const i = document.documentElement,
        o = document.body
      ;((i.style.overflow = r.overflow),
        (i.style.paddingRight = r.paddingRight),
        (o.style.overflow = t.overflow),
        (o.style.paddingRight = t.paddingRight),
        i.classList.remove('yh-popup-parent--hidden'),
        setTimeout(() => {
          i.classList.contains('yh-popup-parent--hidden') ||
            i.style.removeProperty('--yh-scrollbar-width')
        }, 400),
        (_.value = !1))
    }
  return (
    ...
    for (const e of _) {
      ...
      try {
        const o = JSON.parse(i),
          u =
            (n =
              (s = (t = o == null ? void 0 : o.choices) == null ? void 0 : t[0]) == null
                ? void 0
                : s.delta) == null
              ? void 0
              : n.content
    ...
    for (const s of _) {
      ...
      try {
        const e = JSON.parse(n),
          i = (t = e == null ? void 0 : e.output) == null ? void 0 : t.text
        i && (r += i)
      } catch {}
    }
    return r || null
...
`)
    ...
    for (const s of _) {
      ...
      try {
        const e = JSON.parse(n)
        ;(e == null ? void 0 : e.type) === 'content_block_delta' &&
          (t = e == null ? void 0 : e.delta) != null &&
          t.text &&
          (r += e.delta.text)
      } catch {}
    }
          ...
          const l = JSON.parse(u),
            d =
              (i =
                (e =
                  (n =
                    (s = (t = l == null ? void 0 : l.candidates) == null ? void 0 : t[0]) == null
                      ? void 0
                      : s.content) == null
                    ? void 0
                    : n.parts) == null
  ...
  return {
    ...
    fetchStream: async (l, ...d) => {
      ...
      const m = (k) => {
        var w
        k &&
          (e && s ? s.push(k) : ((r.value += k), (w = a.onUpdate) == null || w.call(a, k, r.value)))
      }
      try {
        ...
        if (typeof k == 'object' && k !== null && Symbol.asyncIterator in k)
          ...
          for (;;) {
            ...
            if (j) break
            const b = H.decode(F, { stream: !0 }),
              g = n(b)
            g !== null && m(g)
          }
        }
        t.signal.aborted ||
          (e && s && s.flush(), (_.value = !1), (f = a.onFinish) == null || f.call(a, r.value))
      } catch (k) {
        ;(k.name !== 'AbortError' && ((y = a.onError) == null || y.call(a, k)),
          s == null || s.cancel(),
          (_.value = !1))
      }
    },
...
function Qy(a = {}) {
  ...
  const l = () => {
    if (u && i.value) {
      ...
      ;(k == null ? void 0 : k.role) === 'assistant' &&
        k.status === 'generating' &&
        (k.status = 'stopped')
    }
  }
  return {
    ...
    sendMessage: async (k) => {
      ...
      try {
        const g = await a.request(k, j, u.signal),
          T = e.value.find((C) => C.id === w)
        T.status = 'generating'
        let E = null
        t &&
        ...
        if (typeof g == 'object' && g !== null && Symbol.asyncIterator in g)
          ...
          for (;;) {
            ...
            if (A) break
            const V = z.decode(B, { stream: !0 }),
              D = r(V)
            D && O(D)
          }
        } else typeof g == 'string' && O(g)
        ;(E && E.flush(),
          u.signal.aborted || ((T.status = 'success'), (F = a.onFinish) == null || F.call(a, T)))
      } catch (g) {
...
function tv(a = {}) {
  ...
  const u = P([]),
    ...
    m = (async () => {
      ...
      for (const h of L) Y.find((x) => x.id === h.id) || Y.push(h)
      u.value = Y.sort((h, x) =>
        h.pinned !== x.pinned ? (h.pinned ? -1 : 1) : x.updatedAt - h.updatedAt
      )
    })(),
    f = async () => {
      if (o)
    ...
    y = R(() => {
      ...
      const v = [],
        Y = u.value.filter((h) => h.pinned)
      Y.length > 0 && v.push({ label: 'pinned', items: Y })
      for (const h of Xi) L[h].length > 0 && v.push({ label: h, items: L[h] })
      return v
    }),
    k = R(() => u.value.slice(0, l.value * s)),
    w = R(() => k.value.length < u.value.length),
    H = async () => {
      !w.value ||
        d.value ||
        ((d.value = !0), await new Promise((L) => setTimeout(L, 300)), l.value++, (d.value = !1))
    },
    j = async (L, v) => {
      const Y = {
        id: _(),
    ...
    g = async (L, v = !0) => {
      ;(await b(L, { pinned: v }),
        u.value.sort((Y, h) =>
          Y.pinned !== h.pinned ? (Y.pinned ? -1 : 1) : h.updatedAt - Y.updatedAt
        ),
        await f())
    },
    T = async () => {
    ...
    A = async () => {
      if (!(!n || E.value)) {
        ...
        try {
          ...
          for (const v of L) u.value.find((Y) => Y.id === v.id) || u.value.push(v)
          ;(u.value.sort((v, Y) =>
            v.pinned !== Y.pinned ? (v.pinned ? -1 : 1) : Y.updatedAt - v.updatedAt
          ),
            (O.value = Date.now()),
            await f())
        } catch (L) {
...
function nv(a = {}) {
  ...
  const E = typeof window < 'u' ? window : null,
    O =
      (E == null ? void 0 : E.SpeechRecognition) ||
      (E == null ? void 0 : E.webkitSpeechRecognition),
    C = !!O,
    z = (Y) => {
      b = []
      ...
      ;((Y.lang = _),
        ...
        (Y.onresult = (h) => {
          ...
          for (let q = h.resultIndex; q < h.results.length; ++q)
            h.results[q].isFinal
              ? ((l.value += h.results[q][0].transcript),
                (S = a.onResult) == null || S.call(a, l.value))
              : (x += h.results[q][0].transcript)
    ...
    B = async (Y) => {
      ...
      try {
        ...
        ;((w.value = new x()),
          w.value.state === 'suspended' && (await w.value.resume()),
          (H.value = w.value.createAnalyser()),
          (H.value.fftSize = 256),
          w.value.createMediaStreamSource(Y).connect(H.value))
        const $ = H.value.frequencyBinCount,
          q = new Uint8Array($),
          I = () => {
            if (!u.value) {
      ...
      if (!j.value)
        try {
          ;((l.value = ''),
            ...
            (y.value = null),
            (T = null),
            (j.value = await navigator.mediaDevices.getUserMedia({ audio: !0 })),
            (u.value = !0),
            z(j.value),
            A(),
            await B(j.value),
            (Y = F.value) == null || Y.start(1e3),
            (h = k.value) == null || h.start(),
            (x = a.onStart) == null || x.call(a))
        } catch ($) {
          ;((u.value = !1),
            j.value && (j.value.getTracks().forEach((q) => q.stop()), (j.value = null)),
            console.error('[yh-ui/hooks] useAiVoice start failed:', $),
            (S = a.onError) == null || S.call(a, $))
        }
    },
    D = () => {
      if (j.value) {
    ...
    v = () => {
      ;(g && (cancelAnimationFrame(g), (g = null)),
        w.value &&
          w.value.state !== 'closed' &&
          (w.value.close().catch((Y) => {}), (w.value = null)),
        (f.value = new Array(i).fill(5)),
        (m.value = 0))
    }
  return (
    Kr(() => {
      ...
      : new Promise((_, r) => {
          ...
          ;((t.onerror = () => r(t.error)),
            ...
            (t.onupgradeneeded = (s) => {
              ...
              n.objectStoreNames.contains(this.storeName) ||
                n.createObjectStore(this.storeName, { keyPath: 'key' })
            }))
        })
  }
  async get(_) {
...
class av {
  ...
  async clear() {
    ...
    if (!_.ok && _.status !== 404) throw new Error(`HTTP ${_.status}`)
  }
}
function sv(a = {}) {
  const { storage: _ = new Qi(), conversationKey: r = 'ai-conversations', autoSave: t = !0 } = a,
    s = P([]),
    n = P(null),
    e = P(!1),
    i = P(!1),
    ...
    m = (b) => {
      ...
      g !== -1 &&
        (s.value.splice(g, 1),
        n.value === b && (n.value = s.value.length > 0 ? s.value[s.value.length - 1].id : null),
        t && l())
    },
    f = () => s.value.find((b) => b.id === n.value),
    y = (b) => {
      const g = f()
      if (!g) return
      const T = {
    ...
    H = () => JSON.stringify(s.value, null, 2),
    j = async (b) => {
      try {
        const g = JSON.parse(b)
        if (!Array.isArray(g)) throw new Error('Invalid format: expected array')
        return ((s.value = g), await l(), !0)
      } catch (g) {
        return (
          (o.value = g instanceof Error ? g : new Error('Failed to import conversations')),
          !1
        )
      }
    },
    ...
    _n ||
      ...
      (function (a, _) {
        ;(function (r, t) {
          ...
          var s = t(r),
            n = 'يناير_فبراير_مارس_أبريل_مايو_يونيو_يوليو_أغسطس_سبتمبر_أكتوبر_نوفمبر_ديسمبر'.split(
              '_'
            ),
            e = { 1: '١', 2: '٢', 3: '٣', 4: '٤', 5: '٥', 6: '٦', 7: '٧', 8: '٨', 9: '٩', 0: '٠' },
            i = {
              '١': '1',
              '٢': '2',
              '٣': '3',
      ...
      (function (a, _) {
        ;(function (r, t) {
          ...
          var s = t(r),
            ...
            o = 'студ_лют_сак_крас_трав_чэрв_ліп_жнів_вер_каст_ліст_снеж'.split('_'),
            u = /D[oD]?(\[[^[\]]*\]|\s)+MMMM?/
          function l(y, k, w) {
            var H, j
            return w === 'm'
              ...
              : w === 'h'
                ...
                : y +
                  ...
                  ((H = +y),
                  (j = {
                    ss: k ? 'секунда_секунды_секунд' : 'секунду_секунды_секунд',
                    mm: k ? 'хвіліна_хвіліны_хвілін' : 'хвіліну_хвіліны_хвілін',
                    hh: k ? 'гадзіна_гадзіны_гадзін' : 'гадзіну_гадзіны_гадзін',
                    dd: 'дзень_дні_дзён',
                  ...
                  H % 10 == 1 && H % 100 != 11
                    ? j[0]
                    : H % 10 >= 2 && H % 10 <= 4 && (H % 100 < 10 || H % 100 >= 20)
                      ? j[1]
                      : j[2])
          }
          var d = function (y, k) {
    ...
    pn ||
      ...
      (function (a, _) {
        ;(function (r, t) {
          ...
          var s = t(r),
            n = { 1: '১', 2: '২', 3: '৩', 4: '৪', 5: '৫', 6: '৬', 7: '৭', 8: '৮', 9: '৯', 0: '০' },
            e = {
              '১': '1',
              '২': '2',
              '৩': '3',
      ...
      (function (a, _) {
        ;(function (r, t) {
          ...
          function n(i, o, u, l) {
            ...
            return o
              ? (d[u][2] ? d[u][2] : d[u][1]).replace('%d', i)
              : (l ? d[u][0] : d[u][1]).replace('%d', i)
          }
          var e = {
            name: 'et',
    ...
    ka ||
      ...
      (function (a, _) {
        ;(function (r, t) {
          ...
          ;(n.default.locale(u, null, !0),
            (r.default = u),
            (r.englishToArabicNumbersMap = e),
          ...
          function l(y, k, w) {
            ...
            return w === 'm'
              ...
              : y +
                  ...
                  ((H = +y),
                  (j = {
                    mm: k ? 'минута_минуты_минут' : 'минуту_минуты_минут',
                    hh: 'час_часа_часов',
                    dd: 'день_дня_дней',
        ...
        ;(function (r, t) {
          ...
          function i(u, l, d, m) {
            ...
            switch (d) {
              ...
              case 'mm':
                return n(u)
                  ? f + (l || m ? 'minuti' : 'minutama')
                  : e(u)
                    ? f + (l || m ? 'minute' : 'minutami')
                    : f + (l || m ? 'minut' : 'minutami')
              case 'h':
              ...
              case 'MM':
                return n(u)
                  ? f + (l || m ? 'meseca' : 'mesecema')
                  : e(u)
                    ? f + (l || m ? 'mesece' : 'meseci')
                    : f + (l || m ? 'mesecev' : 'meseci')
              case 'y':
        ...
        ;(function (r, t) {
          ...
          var s = t(r),
            n = {
              ...
              correctGrammarCase: function (i, o) {
            ...
            return f === 'm'
              ...
              : f === 'h'
                ...
                : d +
                  ...
                  ((y = +d),
                  (k = {
                    ss: m ? 'секунда_секунди_секунд' : 'секунду_секунди_секунд',
                    mm: m ? 'хвилина_хвилини_хвилин' : 'хвилину_хвилини_хвилин',
                    hh: m ? 'година_години_годин' : 'годину_години_годин',
                    dd: 'день_дні_днів',
...
export {
  ...
  Ji as FormContextKey,
  Ni as FormItemContextKey,
  Qi as IndexedDBAdapter,
  ev as aiChatTestUtils,
  Xy as aiStreamTestUtils,
  Wi as claudeParser,
  qi as configProviderContextKey,
  zy as createZIndexCounter,
  wi as defaultNamespace,
  Ki as ernieParser,
  Ui as geminiParser,
  Wr as getDayjsLocale,
  $i as getNextZIndex,
  Ii as idInjectionKey,
  Gi as localStorageAdapter,
  bi as namespaceContextKey,
  Oi as openaiParser,
  ...
  Qy as useAiChat,
  tv as useAiConversations,
  sv as useAiPersistence,
  rv as useAiRequest,
  Vy as useAiStream,
  nv as useAiVoice,
  Uy as useCache,
  ...
  Cy as useZIndex,
  ji as zIndexContextKey,
  Ti as zIndexCounterKey
}
