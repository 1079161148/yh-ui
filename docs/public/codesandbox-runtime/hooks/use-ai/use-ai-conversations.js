...
function useAiConversations(options = {}) {
  ...
  const updateConversation = async (id, updates) => {
    const idx = conversations.value.findIndex((c) => c.id === id)
    if (idx !== -1) {
      conversations.value[idx] = __spreadProps(
        __spreadValues(__spreadValues({}, conversations.value[idx]), updates),
        {
          updatedAt: Date.now()
        }
      )
...
export { AiIndexedDBAdapter, localStorageAdapter, useAiConversations }
