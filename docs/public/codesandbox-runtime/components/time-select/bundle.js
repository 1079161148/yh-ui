...
var formatMinutesToTime = (minutes, format = "HH:mm") => {
  ...
  const mm2 = String(mins).padStart(2, "0");
  return format.replace("HH", HH).replace("H", String(hours)).replace("mm", mm2).replace("m", String(mins));
};
...
var generateTimeOptions = (start, end, step, format = "HH:mm", _includeEnd = false) => {
  ...
  for (let m2 = startMinutes; m2 <= endMinutes; m2 += stepMinutes) {
    if (m2 > 24 * 60) break;
    const value = formatMinutesToTime(m2, "HH:mm");
    const label = formatMinutesToTime(m2, format);
...
var __sfc__ = /* @__PURE__ */ Object.assign({
  ...
  setup(__props, { expose: __expose, emit: __emit }) {
    ...
    const hasValue = computed11(() => props.modelValue !== void 0 && props.modelValue !== "");
    ...
    const scrollToSelected = () => {
      ...
      nextTick(() => {
        var _a2;
        const selectedEl = (_a2 = optionsRef.value) == null ? void 0 : _a2.querySelector(
