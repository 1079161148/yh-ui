...
const useVirtualScroll = (options) => {
  ...
  const mergedConfig = computed(() => __spreadValues(__spreadValues({}, defaultConfig), config.value));
  ...
  const realTotalHeight = computed(() => {
    ...
    return ((_a = rowPositions.value[rowPositions.value.length - 1]) == null ? void 0 : _a.bottom) || 0;
  });
  ...
  const offsetTop = computed(() => {
    ...
    const positions = rowPositions.value;
    return ((_a = positions[visibleRange.value.start]) == null ? void 0 : _a.top) || 0;
  });
