...
import { toDisplayString as _toDisplayString, normalizeClass as _normalizeClass, normalizeStyle as _normalizeStyle, openBlock as _openBlock, createElementBlock as _createElementBlock, createCommentVNode as _createCommentVNode, createElementVNode as _createElementVNode, Transition as _Transition, withCtx as _withCtx, createVNode as _createVNode, renderList as _renderList, Fragment as _Fragment, renderSlot as _renderSlot, createTextVNode as _createTextVNode, withModifiers as _withModifiers } from "vue";
import { ref as ref12, computed as computed11, onMounted as onMounted3, onUnmounted as onUnmounted4, watch as watch4 } from "vue";
...
var __sfc__ = /* @__PURE__ */ Object.assign({ name: "YhProductCard" }, {
  ...
  setup(__props, { expose: __expose, emit: __emit }) {
    ...
    const cardRef = ref12(null);
    let observer = null;
    const hasExposed = ref12(false);
    const setupObserver = () => {
    ...
    onMounted3(setupObserver);
    onUnmounted4(() => observer == null ? void 0 : observer.disconnect());
    const currentImage = ref12(props.image);
    const isHovering = ref12(false);
    const videoActive = ref12(false);
    watch4(
      () => props.image,
      (val) => {
    ...
    const displayPrice = computed11(() => {
      const p = Number(props.price);
      return isNaN(p) ? props.price : p.toFixed(2);
    });
    const displayMarketPrice = computed11(() => {
      const p = Number(props.marketPrice);
      return isNaN(p) ? props.marketPrice : p.toFixed(2);
    });
    const displayVipPrice = computed11(() => {
      const p = Number(props.vipPrice);
      return isNaN(p) ? props.vipPrice : p.toFixed(2);
    });
    const vipLabelText = computed11(() => props.vipLabel || t("productcard.vip"));
    const soldOutText = computed11(() => t("productcard.soldOut"));
    const actionText = computed11(() => props.actionText || t("productcard.buyNow"));
    const stockStyle = computed11(() => ({
    ...
    const badgeImageErrors = ref12({});
    const handleBadgeImageError = (idx) => {
      badgeImageErrors.value[idx] = true;
    };
    const __returned__ = { props, emit, ns, t, themeStyle, cardRef, get observer() {
      return observer;
    }, set observer(v) {
      observer = v;
    }, hasExposed, setupObserver, currentImage, isHovering, videoActive, handleMouseEnter, handleMouseLeave, displayPrice, displayMarketPrice, displayVipPrice, vipLabelText, soldOutText, actionText, stockStyle, handleAction, handleClick, badgeImageErrors, handleBadgeImageError, ref: ref12, computed: computed11, onMounted: onMounted3, onUnmounted: onUnmounted4, watch: watch4, get useNamespace() {
      return useNamespace;
    }, get useLocale() {
      return useLocale;
