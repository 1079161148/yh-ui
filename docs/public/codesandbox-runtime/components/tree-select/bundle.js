// docs/public/codesandbox-runtime/components/tree-select/src/tree-select.js
import {
...
// docs/public/codesandbox-runtime/components/tree-select/src/tree-select-meta.js
var treeSelectProps = {
  // 基础属性
  modelValue: {
...
// docs/public/codesandbox-runtime/components/tree-select/src/use-tree.js
...
var useTree = (props, emit) => {
  ...
  const createTreeNode = (data, parent, level = 0) => {
    ...
    const key =
      (_a2 = data[keyProp]) != null ? _a2 : `node-${Math.random().toString(36).slice(2, 9)}`
    const node = reactive({
      key,
  ...
  const flatData = computed11(() => {
    ...
    const walk = (nodes) => {
      nodes.forEach((node) => {
        ...
        if (node.visible) {
          result.push(node)
          if (node.expanded && ((_a2 = node.children) == null ? void 0 : _a2.length))
            walk(node.children)
        }
      })
    }
