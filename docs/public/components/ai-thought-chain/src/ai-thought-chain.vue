...
<template>
  <div
    :class="[ns.b(), ns.is('expanded', isExpanded), ns.is('thinking', currentStatus === 'thinking' || currentStatus === 'loading'), ns.is('timeline', itemsToUse && itemsToUse.length > 0), ns.is('draggable', draggable), className]"
    :style="[themeStyle, style]"
  >
    <!-- 进度条 -->
    <div v-if="showProgress && itemsToUse && itemsToUse.length > 0" :class="ns.e('progress-bar')">
      <div :class="ns.e('progress-fill')" :style="{
  width: currentProgress + '%'
    ...
    <template v-if="itemsToUse && itemsToUse.length > 0">
      <div
        ...
        :key="item.id || index"
        :class="[ns.e('item'), ns.is('last', index === items.length - 1), ns.is('active', item.status === 'thinking' || item.status === 'loading'), ns.is('drag-over', dragOverIndex === index), ns.is('dragging', draggedIndex === index)]"
        :draggable="draggable"
  ...
  'is-gradient': lineGradient
        ...
        <div :class="ns.e('item-main')">
          <div :class="ns.e('item-header')" @click="toggleItemExpand(index)">
            ...
            <!-- 节点进度 -->
            <span v-if="item.progress !== void 0" :class="ns.e('item-progress')">
              {{ item.progress }}%
            </span>
            ...
            <YhIcon
              ...
              name="arrow-down"
              :class="[ns.e('item-arrow'), {
  'is-expanded': itemExpandedStates[index]
}]"
            />
          </div>
  ...
  'is-loading': currentStatus === 'thinking' || currentStatus === 'loading'
        ...
        <YhIcon name="arrow-down" :class="[ns.e('arrow'), {
...
.yh-ai-thought-chain {
  margin: 12px 0;
  border-left: 3px solid var(--yh-color-info-light-5);
  padding-left: 12px;
  max-width: 100%;
...
.yh-ai-thought-chain__progress-fill {
  height: 100%;
  background: linear-gradient(90deg, var(--yh-color-primary), var(--yh-color-success));
  border-radius: 2px;
...
.yh-ai-thought-chain.is-draggable .yh-ai-thought-chain__item.is-dragging {
  opacity: 0.5;
}
.yh-ai-thought-chain.is-draggable .yh-ai-thought-chain__item.is-drag-over {
  background: var(--yh-color-primary-light-9);
}

.yh-ai-thought-chain.is-draggable .yh-ai-thought-chain__drag-handle {
  margin-right: 8px;
  cursor: grab;
  color: var(--yh-text-color-placeholder);
  font-size: 12px;
}
.yh-ai-thought-chain.is-draggable .yh-ai-thought-chain__drag-handle:active {
  cursor: grabbing;
}
...
.yh-ai-thought-chain__item-header:hover .yh-ai-thought-chain__item-actions {
  opacity: 1;
}

.yh-ai-thought-chain__action-icon {
  display: inline-flex;
  align-items: center;
...
.yh-ai-thought-chain__action-icon:hover {
...
.yh-ai-thought-chain__add-node:hover {
  color: var(--yh-color-primary);
  background: var(--yh-fill-color-light);
}

.yh-ai-thought-chain__header {
  display: flex;
  align-items: center;
  cursor: pointer;
  font-size: 13px;
...
.yh-ai-thought-chain__header:hover {
  color: var(--yh-color-primary);
}

.yh-ai-thought-chain__icon {
  margin-right: 8px;
  display: flex;
  align-items: center;
  justify-content: center;
...
.yh-ai-thought-chain__status-icon {
  font-size: 14px;
  color: var(--yh-color-success);
}
.yh-ai-thought-chain__status-icon.is-loading {
  color: var(--yh-color-primary);
  animation: yh-spin 1s linear infinite;
}

.yh-ai-thought-chain__title {
  font-weight: 500;
  flex: 1;
}

.yh-ai-thought-chain__arrow {
  transition: transform 0.3s;
  margin-left: 8px;
}
.yh-ai-thought-chain__arrow.is-expanded {
...
.yh-ai-thought-chain__text {
  white-space: pre-wrap;
  word-break: break-all;
}

.yh-ai-thought-chain.is-timeline {
  border-left: none;
  padding-left: 0;
}
.yh-ai-thought-chain.is-timeline .yh-ai-thought-chain__item {
  display: flex;
  gap: 12px;
  position: relative;
  padding-bottom: 20px;
}
.yh-ai-thought-chain.is-timeline .yh-ai-thought-chain__item.is-last {
  padding-bottom: 0;
}

.yh-ai-thought-chain.is-timeline .yh-ai-thought-chain__item .yh-ai-thought-chain__item-dot-wrapper {
  display: flex;
...
.yh-ai-thought-chain.is-timeline .yh-ai-thought-chain__item .yh-ai-thought-chain__item-dot {
  display: flex;
  align-items: center;
  justify-content: center;
  border-radius: 50%;
...
.yh-ai-thought-chain.is-timeline .yh-ai-thought-chain__item .yh-ai-thought-chain__item-dot--thinking {
  color: var(--yh-color-primary);
}
.yh-ai-thought-chain.is-timeline .yh-ai-thought-chain__item .yh-ai-thought-chain__item-dot--thinking .yh-icon.is-loading {
  animation: yh-spin 1s linear infinite;
}

.yh-ai-thought-chain.is-timeline .yh-ai-thought-chain__item .yh-ai-thought-chain__item-dot--loading {
  color: var(--yh-color-primary);
}
.yh-ai-thought-chain.is-timeline .yh-ai-thought-chain__item .yh-ai-thought-chain__item-dot--loading .yh-icon.is-loading {
  animation: yh-spin 1s linear infinite;
}

.yh-ai-thought-chain.is-timeline .yh-ai-thought-chain__item .yh-ai-thought-chain__item-dot--success {
  color: var(--yh-color-success);
}

.yh-ai-thought-chain.is-timeline .yh-ai-thought-chain__item .yh-ai-thought-chain__item-dot--complete {
  color: var(--yh-color-success);
}

.yh-ai-thought-chain.is-timeline .yh-ai-thought-chain__item .yh-ai-thought-chain__item-dot--error {
  color: var(--yh-color-danger);
}

.yh-ai-thought-chain.is-timeline .yh-ai-thought-chain__item .yh-ai-thought-chain__item-dot--none {
  color: var(--yh-color-info-light-3);
}

.yh-ai-thought-chain.is-timeline .yh-ai-thought-chain__item .yh-ai-thought-chain__item-dot--small {
  width: 14px;
  height: 14px;
  font-size: 12px;
}

.yh-ai-thought-chain.is-timeline .yh-ai-thought-chain__item .yh-ai-thought-chain__item-dot--default {
  width: 20px;
  height: 20px;
  font-size: 14px;
}

.yh-ai-thought-chain.is-timeline .yh-ai-thought-chain__item .yh-ai-thought-chain__item-dot--large {
  width: 26px;
  height: 26px;
  font-size: 18px;
}

.yh-ai-thought-chain.is-timeline .yh-ai-thought-chain__item .yh-ai-thought-chain__item-line {
  position: absolute;
  top: 20px;
  bottom: 0;
  left: 9px;
...
.yh-ai-thought-chain.is-timeline .yh-ai-thought-chain__item .yh-ai-thought-chain__item-line.is-gradient {
  background: linear-gradient(to bottom, var(--yh-color-primary), var(--yh-border-color-lighter));
}

.yh-ai-thought-chain.is-timeline .yh-ai-thought-chain__item .yh-ai-thought-chain__item-main {
  flex: 1;
}

.yh-ai-thought-chain.is-timeline .yh-ai-thought-chain__item .yh-ai-thought-chain__item-header {
  display: flex;
  align-items: center;
  cursor: pointer;
  font-size: 14px;
...
.yh-ai-thought-chain.is-timeline .yh-ai-thought-chain__item .yh-ai-thought-chain__item-header:hover {
  color: var(--yh-color-primary);
}
...
.yh-ai-thought-chain.is-timeline .yh-ai-thought-chain__item .yh-ai-thought-chain__item-arrow.is-expanded {
  transform: rotate(180deg);
}

.yh-ai-thought-chain.is-timeline .yh-ai-thought-chain__item .yh-ai-thought-chain__item-content {
  margin-top: 8px;
  padding: 10px;
  background: var(--yh-fill-color-light);
  border-radius: 6px;
...
.yh-ai-thought-chain.is-timeline .yh-ai-thought-chain__item .yh-ai-thought-chain__item-text {
  white-space: pre-wrap;
}
...
.yh-ai-thought-collapse-enter-from,
.yh-ai-thought-collapse-leave-to {
  max-height: 0;
  opacity: 0;
