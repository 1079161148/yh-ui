...
const useTree = (props, emit) => {
  const nodeMap = (0, _vue.ref)(/* @__PURE__ */new Map());
  const treeData = (0, _vue.ref)([]);
  const mapVersion = (0, _vue.ref)(0);
  const alias = (0, _vue.computed)(() => ({
  ...
  const createTreeNode = (data, parent, level = 0) => {
    ...
    const key = data[keyProp] ?? `node-${Math.random().toString(36).slice(2, 9)}`;
    const node = (0, _vue.reactive)({
      key,
  ...
  const flatData = (0, _vue.computed)(() => {
  ...
  const syncCheckState = val => {
    const keys = new Set(Array.isArray(val) ? val : val !== void 0 && val !== null && val !== "" ? [val] : []);
  ...
  (0, _vue.watch)(() => props.data, syncData, {
    immediate: true,
    deep: true
  });
  (0, _vue.watch)(() => props.modelValue, val => syncCheckState(val), {
    deep: true
  });
  return {
    flatData,
