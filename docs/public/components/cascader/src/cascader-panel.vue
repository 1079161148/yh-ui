...
<template>
    ...
    >
      <template #default="{ option, level: menuLevel, startIndex, idx }">
        <div
          ...
          :class="[ns.e('node'), ns.is('expanded', isExpanded(option, menuLevel)), ns.is('checked', isChecked(getPath(option, menuLevel))), ns.is('disabled', option[config.disabled]), ns.is('selectable', isSelectable(option))]"
          :style="virtual ? {
  position: 'absolute',
  ...
  height: `${itemHeight}px`
        ...
        >
          ...
          >
            <span
              :class="[ns.e('checkbox-inner'), ns.is('checked', isChecked(getPath(option, menuLevel)))]"
            >
              <svg
