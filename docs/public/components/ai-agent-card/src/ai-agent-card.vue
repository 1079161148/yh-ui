  ...
  >
    ...
    <div :class="ns.e('body')">
      ...
      <div v-if="showStats && data.stats" :class="ns.e('stats')">
        <span v-if="data.stats.uses !== void 0" :class="ns.e('stat-item')">
          <YhIcon name="chart-bar" />
          {{ formattedUses }} {{ t("ai.agent.uses") }}
        </span>
        <span v-if="data.stats.rating !== void 0" :class="ns.e('stat-item')">
        ...
        <span v-if="data.stats.responseTime !== void 0" :class="ns.e('stat-item')">
          <YhIcon name="time" />
...
.yh-ai-agent-card {
  ...
  border: 1.5px solid var(--yh-border-color-light);
  border-radius: var(--yh-border-radius-large, 12px);
  cursor: pointer;
  transition: all 0.25s cubic-bezier(0.4, 0, 0.2, 1);
  overflow: hidden;
}
.yh-ai-agent-card::before {
  content: "";
  position: absolute;
  inset: 0;
  background: linear-gradient(135deg, var(--yh-color-primary-light-9, rgba(64, 158, 255, 0.04)) 0%, transparent 60%);
  opacity: 0;
  transition: opacity 0.25s;
  pointer-events: none;
}
.yh-ai-agent-card:hover {
  border-color: var(--yh-color-primary-light-5, #79bbff);
  box-shadow: 0 8px 24px rgba(0, 0, 0, 0.1), 0 0 0 1px var(--yh-color-primary-light-6, #a0cfff);
  transform: translateY(-2px);
}
.yh-ai-agent-card:hover::before {
  opacity: 1;
}
.yh-ai-agent-card:hover .yh-ai-agent-card__overlay-actions {
  opacity: 1;
}

.yh-ai-agent-card:hover .yh-ai-agent-card__use-btn {
  background: var(--yh-color-primary);
  color: #fff;
}

.yh-ai-agent-card.is-selected {
  border-color: var(--yh-color-primary);
  box-shadow: 0 0 0 2px var(--yh-color-primary-light-6, #a0cfff);
}
.yh-ai-agent-card.is-selected::before {
  opacity: 1;
}
.yh-ai-agent-card--horizontal {
  flex-direction: row;
  align-items: flex-start;
}
.yh-ai-agent-card--horizontal .yh-ai-agent-card__avatar-wrapper {
  flex-shrink: 0;
}

.yh-ai-agent-card--horizontal .yh-ai-agent-card__body {
  flex: 1;
  min-width: 0;
}

.yh-ai-agent-card--horizontal .yh-ai-agent-card__actions {
  flex-shrink: 0;
  flex-direction: column;
  align-self: center;
  padding-right: 32px;
...
.yh-ai-agent-card--horizontal .yh-ai-agent-card__overlay-actions {
  flex-direction: column;
  top: 50%;
  right: 8px;
  transform: translateY(-50%);
}
.yh-ai-agent-card--horizontal .yh-ai-agent-card__overlay-actions button {
  width: 24px;
  height: 24px;
  font-size: 13px;
  background: var(--yh-bg-color);
...
.yh-ai-agent-card--compact {
  padding: 12px 16px;
  flex-direction: row;
  align-items: center;
  gap: 12px;
}
.yh-ai-agent-card--compact .yh-ai-agent-card__description {
  display: none;
}

.yh-ai-agent-card--compact .yh-ai-agent-card__stats {
  display: flex;
  margin-top: 0;
  gap: 12px;
}
.yh-ai-agent-card--compact .yh-ai-agent-card__stats .yh-icon {
  font-size: 13px;
}

.yh-ai-agent-card--compact .yh-ai-agent-card__capabilities {
  display: none;
}

.yh-ai-agent-card--compact .yh-ai-agent-card__actions {
  margin-top: 0;
  flex-shrink: 0;
  padding-right: 32px;
}

.yh-ai-agent-card--compact .yh-ai-agent-card__overlay-actions {
  flex-direction: column;
  top: 50%;
  right: 8px;
  transform: translateY(-50%);
}
.yh-ai-agent-card--compact .yh-ai-agent-card__overlay-actions button {
  width: 24px;
  height: 24px;
  border-radius: 5px;
  font-size: 12px;
...
.yh-ai-agent-card__overlay-actions {
  position: absolute;
  top: 12px;
  right: 12px;
  display: flex;
...
.yh-ai-agent-card__overlay-actions button {
  display: flex;
  align-items: center;
  ...
  border: none;
  border-radius: 6px;
  background: var(--yh-bg-color-overlay, rgba(255, 255, 255, 0.85));
...
.yh-ai-agent-card__overlay-actions button:hover {
  background: var(--yh-color-primary-light-9);
  color: var(--yh-color-primary);
}

.yh-ai-agent-card__fav-btn.is-active {
  color: var(--yh-color-warning) !important;
}

.yh-ai-agent-card__avatar-wrapper {
  position: relative;
  width: fit-content;
}

.yh-ai-agent-card__avatar {
  width: 56px;
  height: 56px;
  border-radius: 14px;
  background: linear-gradient(135deg, var(--yh-color-primary-light-8), var(--yh-color-primary-light-5));
  display: flex;
  align-items: center;
  justify-content: center;
  font-size: 28px;
...
.yh-ai-agent-card__status-dot {
  ...
  height: 12px;
  border-radius: 50%;
  border: 2px solid var(--yh-bg-color);
}
.yh-ai-agent-card__status-dot.is-online {
  background: var(--yh-color-success);
}
.yh-ai-agent-card__status-dot.is-busy {
  background: var(--yh-color-warning);
}
.yh-ai-agent-card__status-dot.is-offline {
  background: var(--yh-text-color-disabled);
}

.yh-ai-agent-card__body {
  flex: 1;
  min-width: 0;
  display: flex;
  flex-direction: column;
...
.yh-ai-agent-card__header {
  display: flex;
  flex-direction: column;
  gap: 2px;
}

.yh-ai-agent-card__name-row {
  display: flex;
  align-items: center;
  gap: 6px;
}

.yh-ai-agent-card__name {
  font-size: 16px;
  font-weight: 600;
...
.yh-ai-agent-card__verified-icon {
  color: var(--yh-color-primary);
  font-size: 16px;
  flex-shrink: 0;
}

.yh-ai-agent-card__model {
  font-size: 12px;
  color: var(--yh-text-color-secondary);
}

.yh-ai-agent-card__description {
  font-size: 13px;
  color: var(--yh-text-color-regular);
  line-height: 1.6;
  overflow: hidden;
...
.yh-ai-agent-card__capabilities {
  display: flex;
  flex-wrap: wrap;
  gap: 6px;
}

.yh-ai-agent-card__capability-tag {
  display: inline-flex;
  align-items: center;
  gap: 4px;
  padding: 2px 8px;
  ...
  background: var(--yh-color-primary-light-9);
  color: var(--yh-color-primary);
  border: 1px solid var(--yh-color-primary-light-6);
}
.yh-ai-agent-card__capability-tag--primary {
  background: var(--yh-color-primary-light-9);
  color: var(--yh-color-primary);
  border-color: var(--yh-color-primary-light-6);
}

.yh-ai-agent-card__capability-tag--success {
  background: var(--yh-color-success-light-9);
  color: var(--yh-color-success);
  border-color: var(--yh-color-success-light-5);
}

.yh-ai-agent-card__capability-tag--warning {
  background: var(--yh-color-warning-light-9);
  color: var(--yh-color-warning);
  border-color: var(--yh-color-warning-light-5);
}

.yh-ai-agent-card__capability-tag--danger {
  background: var(--yh-color-danger-light-9);
  color: var(--yh-color-danger);
  border-color: var(--yh-color-danger-light-5);
}

.yh-ai-agent-card__capability-tag--info {
  background: var(--yh-color-info-light-9);
  color: var(--yh-color-info);
  border-color: var(--yh-color-info-light-5);
}

.yh-ai-agent-card__capability-more {
  padding: 2px 8px;
  border-radius: 100px;
  font-size: 11px;
...
.yh-ai-agent-card__stat-item .yh-icon {
  font-size: 13px;
}

.yh-ai-agent-card__stars {
  display: flex;
  gap: 1px;
}
.yh-ai-agent-card__stars .yh-icon {
  font-size: 12px;
  color: var(--yh-text-color-disabled);
}
.yh-ai-agent-card__stars .yh-icon.is-filled {
  color: var(--yh-color-warning);
}

.yh-ai-agent-card__review-count {
  color: var(--yh-text-color-disabled);
  font-size: 11px;
}

.yh-ai-agent-card__actions {
  display: flex;
  gap: 8px;
  margin-top: 4px;
}

.yh-ai-agent-card__use-btn {
  display: flex;
  align-items: center;
  justify-content: center;
  gap: 6px;
  width: 100%;
  padding: 8px 16px;
  border: 1.5px solid var(--yh-color-primary-light-5, #79bbff);
  border-radius: var(--yh-border-radius-base, 6px);
...
.yh-ai-agent-card__use-btn:hover {
  background: var(--yh-color-primary);
  color: #fff;
  border-color: var(--yh-color-primary);
}

.yh-ai-agent-card.is-loading {
  cursor: default;
  pointer-events: none;
}
.yh-ai-agent-card__skeleton-avatar {
  width: 56px;
  height: 56px;
  border-radius: 14px;
  background: linear-gradient(90deg, var(--yh-fill-color) 25%, var(--yh-fill-color-light) 50%, var(--yh-fill-color) 75%);
  background-size: 200% 100%;
  animation: yh-agent-shimmer 1.5s infinite;
}

.yh-ai-agent-card__skeleton-lines {
  flex: 1;
  display: flex;
  flex-direction: column;
  gap: 12px;
...
.yh-ai-agent-card__skeleton-line {
  height: 14px;
  border-radius: 7px;
  width: 100%;
  background: linear-gradient(90deg, var(--yh-fill-color) 25%, var(--yh-fill-color-light) 50%, var(--yh-fill-color) 75%);
