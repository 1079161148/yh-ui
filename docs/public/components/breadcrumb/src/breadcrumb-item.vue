...
const router = computed(() => {
  const globalRouter = instance?.appContext.config.globalProperties?.$router;
...
.yh-breadcrumb > span:last-child .yh-breadcrumb-item__separator {
  display: none;
}
.yh-breadcrumb > span:last-child .yh-breadcrumb-item__link {
  color: var(--yh-text-color-primary);
  font-weight: 500;
  cursor: default;
}
.yh-breadcrumb > span:last-child .yh-breadcrumb-item__link:hover {
  color: var(--yh-text-color-primary);
}
...
.yh-breadcrumb-item__link {
  transition: color 0.2s cubic-bezier(0.645, 0.045, 0.355, 1);
  color: var(--yh-text-color-regular);
  cursor: default;
}
.yh-breadcrumb-item__link.is-link {
  font-weight: 500;
  cursor: pointer;
}
.yh-breadcrumb-item__link.is-link:hover {
  color: var(--yh-color-primary);
}

.yh-breadcrumb-item__separator {
  margin: 0 8px;
  color: var(--yh-text-color-placeholder);
  display: inline-flex;
  align-items: center;
}
.yh-breadcrumb-item__separator svg {
  width: 1em;
  height: 1em;
}
</style>
