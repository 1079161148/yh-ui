...
const vLoading = exports.vLoading = {
  mounted(el, binding) {
    ...
    if (binding.value) {
      el[INSTANCE_KEY] = _service.Loading.service(parseOptions());
    }
  },
  updated(el, binding) {
    if (binding.oldValue !== binding.value) {
      if (binding.value) {
        ...
        el[INSTANCE_KEY] = _service.Loading.service(options);
      } else {
