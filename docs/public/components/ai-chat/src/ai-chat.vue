...
<template>
  ...
  overflow: 'auto'
      ...
      @scroll="virtualScrollEnabled ? onScroll($event) : void 0"
    >
      <div
  ...
  transform: `translateY(${offsetY}px)`
      ...
      <slot
        name="loading"
        v-if="props.loading && (!props.messages.length || props.messages[props.messages.length - 1].role !== 'assistant' || props.messages[props.messages.length - 1].status !== 'generating' && props.messages[props.messages.length - 1].status !== 'loading')"
      >
