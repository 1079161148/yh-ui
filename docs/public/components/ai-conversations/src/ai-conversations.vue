<template>
  <div :class="[ns.b(), ns.is('loading', loading)]" :style="themeStyle">
    ...
    <div
      ...
      :style="listStyle"
      @scroll.passive="virtualScroll ? handleVirtualScroll($event) : void 0"
    >
      <!-- 时间分组渲染 -->
  ...
  right: 0
              ...
              <div
                ...
                :class="[ns.e('item'), ns.is('active', activeId === item.id), ns.is('pinned', !!item.pinned)]"
                :style="{
  height: virtualScrollItemHeight + 'px'
}"
                @click="handleClick(item)"
              >
                <span v-if="item.pinned" :class="ns.e('pin-icon')">
...
.yh-ai-conversations__list.is-virtual {
  flex: none;
  display: block;
  padding: 0;
  position: relative;
...
.yh-ai-conversations__item.is-active::before {
  content: "";
  position: absolute;
  left: 0;
  top: 20%;
...
.yh-ai-conversations__item.is-active .yh-ai-conversations__time,
.yh-ai-conversations__item.is-active .yh-ai-conversations__excerpt {
...
.yh-ai-conversations__item.is-pinned::before {
  content: "";
  position: absolute;
  left: 0;
  top: 25%;
  bottom: 25%;
  width: 3px;
  background-color: var(--yh-color-primary-light-3);
  border-radius: 0 2px 2px 0;
}
