...
<template>
  <div
    :class="[ns.b(), ns.is(status), ns.m(variant), ns.is('selectable', selectable), ns.is('selected', selected)]"
    :style="themeStyle"
    ...
    <div :class="ns.e('content')">
      ...
      <div :class="ns.e('main-side')">
        <div :class="ns.e('info')">
          ...
          <!-- 疯抢进度条 -->
          <div v-if="percent !== void 0" :class="ns.e('progress')">
            <div :class="ns.e('progress-bar')">
              <div :class="ns.e('progress-inner')" :style="{
  width: percent + '%'
}"></div>
            </div>
...
.yh-coupon-card {
  --yh-coupon-primary-color: var(--yh-color-primary);
  --yh-coupon-gray-color: var(--yh-text-color-placeholder);
  --yh-coupon-bg: var(--yh-bg-color-overlay);
  --yh-coupon-border-radius: 12px;
  --yh-coupon-amount-size: 28px;
  --yh-coupon-title-size: 16px;
  --yh-coupon-desc-size: 12px;
  position: relative;
  width: 100%;
  min-height: 100px;
  ...
  background-color: var(--yh-coupon-bg);
  border: 1px solid var(--yh-border-color-light);
  border-radius: var(--yh-coupon-border-radius);
  overflow: hidden;
  box-shadow: var(--yh-shadow-light);
  cursor: pointer;
  transition: transform var(--yh-transition-base), box-shadow var(--yh-transition-base);
}
.yh-coupon-card * {
  box-sizing: border-box;
}
.yh-coupon-card__content {
  display: flex;
  width: 100%;
  height: 100px;
  flex-shrink: 0;
...
.yh-coupon-card.is-selectable:hover {
...
.yh-coupon-card::before, .yh-coupon-card::after {
  content: "";
  position: absolute;
  width: 20px;
  height: 20px;
...
.yh-coupon-card--indent::before {
  top: 50%;
  left: 0;
  transform: translate(-50%, -50%);
}
.yh-coupon-card--indent::after {
  top: 50%;
  right: 0;
  transform: translate(50%, -50%);
}

.yh-coupon-card--circle::before, .yh-coupon-card--circle::after {
  display: block;
}

.yh-coupon-card--scallop::before, .yh-coupon-card--scallop::after {
  display: none;
}
.yh-coupon-card--scallop {
  mask: radial-gradient(circle at 0 20px, transparent 8px, black 9px) 0 0/100% 20px repeat-y;
  -webkit-mask: radial-gradient(circle at 0 20px, transparent 8px, black 9px) 0 0/100% 20px repeat-y;
}

.yh-coupon-card__amount-side {
  width: 30%;
  height: 100%;
  min-height: 100px;
  display: flex;
  ...
  align-items: center;
  background-color: var(--yh-coupon-primary-color);
  color: white;
  position: relative;
  padding: 0 8px;
}
.yh-coupon-card__amount-side::after {
  content: "";
  position: absolute;
  top: 10px;
  bottom: 10px;
...
.yh-coupon-card__amount {
  font-size: var(--yh-coupon-amount-size);
  font-weight: var(--yh-font-weight-bold);
  line-height: 1;
  display: flex;
  align-items: baseline;
}
.yh-coupon-card__symbol {
  font-size: 14px;
  margin-right: 2px;
  font-weight: var(--yh-font-weight-normal);
}

.yh-coupon-card__threshold {
  font-size: 10px;
  margin-top: 4px;
  opacity: 0.9;
}

.yh-coupon-card__main-side {
  flex: 1;
  height: 100%;
  min-height: 100px;
  display: flex;
...
.yh-coupon-card__title {
  margin: 0;
  font-size: var(--yh-coupon-title-size);
...
.yh-coupon-card__description {
  margin: 0;
  font-size: var(--yh-coupon-desc-size);
...
.yh-coupon-card__valid-period {
  font-size: 10px;
  color: var(--yh-text-color-placeholder);
  white-space: nowrap;
}

.yh-coupon-card__action-area {
  z-index: 5;
}

.yh-coupon-card__action-btn {
  padding: 2px 8px;
  ...
  font-size: 10px;
  cursor: pointer;
  transition: opacity var(--yh-transition-base);
}
.yh-coupon-card__action-btn:hover {
  opacity: 0.8;
}
...
.yh-coupon-card.is-used .yh-coupon-card__amount-side, .yh-coupon-card.is-expired .yh-coupon-card__amount-side {
  background-color: var(--yh-coupon-gray-color);
}

.yh-coupon-card.is-used .yh-coupon-card__action-btn, .yh-coupon-card.is-expired .yh-coupon-card__action-btn {
  background-color: var(--yh-fill-color-darker);
  color: var(--yh-text-color-placeholder);
  cursor: not-allowed;
}

.yh-coupon-card.is-used .yh-coupon-card__title, .yh-coupon-card.is-expired .yh-coupon-card__title {
  color: var(--yh-text-color-placeholder);
}

.yh-coupon-card__seal {
  position: absolute;
  top: -10px;
  right: -10px;
  width: 60px;
  height: 60px;
  border: 2px solid var(--yh-border-color);
  border-radius: 50%;
  display: flex;
  justify-content: center;
  align-items: center;
...
.yh-coupon-card__badge--primary {
  background-color: var(--yh-color-primary);
}

.yh-coupon-card__badge--warning {
  background-color: var(--yh-color-warning);
}

.yh-coupon-card__badge--success {
  background-color: var(--yh-color-success);
}

.yh-coupon-card__badge--danger {
  background-color: var(--yh-color-danger);
}

.yh-coupon-card__progress {
  display: flex;
  align-items: center;
  gap: 8px;
  margin-top: 4px;
...
.yh-coupon-card__progress-inner {
  height: 100%;
  background: linear-gradient(90deg, var(--yh-color-danger-light-3), var(--yh-color-danger));
  border-radius: 2px;
  transition: width 0.3s ease;
}

.yh-coupon-card__progress-text {
  font-size: 10px;
  color: var(--yh-color-danger);
  white-space: nowrap;
  line-height: 1;
...
.yh-coupon-card__rules {
  background-color: var(--yh-bg-color-overlay);
  border-top: 1px dashed var(--yh-border-color-light);
  font-size: 10px;
  color: var(--yh-text-color-secondary);
  width: 100%;
  border-bottom-left-radius: var(--yh-coupon-border-radius);
  border-bottom-right-radius: var(--yh-coupon-border-radius);
}

.yh-coupon-card__rules-trigger {
  padding: 8px 12px;
  display: flex;
  justify-content: space-between;
  align-items: center;
  cursor: pointer;
  user-select: none;
  transition: background-color var(--yh-transition-base);
}
.yh-coupon-card__rules-trigger:hover {
  background-color: var(--yh-fill-color-light);
}

.yh-coupon-card__rules-icon {
  transition: transform var(--yh-transition-base);
}
.yh-coupon-card__rules-icon.is-expanded {
  transform: rotate(180deg);
}

.yh-coupon-card__rules-content {
  padding: 0 12px 12px;
  line-height: 1.5;
}
</style>
