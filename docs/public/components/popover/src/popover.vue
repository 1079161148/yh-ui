    ...
    <template #content>
      <div :class="ns.e('content')" :style="contentStyle">
        ...
        <div :class="[ns.e('main'), ns.is('has-icon', !!icon || !!$slots.icon)]">
          <!-- 侧边图标栏 -->
          <div v-if="icon || $slots.icon" :class="ns.e('icon')" :style="{
  color: iconColor
}">
            <slot name="icon">
              <YhIcon :name="icon" />
...
/* 
  ...
  自动感知 html.dark 环境变色
*/
.yh-popover.yh-tooltip__popper {
  border-radius: var(--yh-radius-lg);
  box-shadow: var(--yh-shadow-lg);
  max-width: 400px;
  /* 自动感应暗黑模式阴影 */
}
:root.dark .yh-popover.yh-tooltip__popper, html.dark .yh-popover.yh-tooltip__popper, body.dark .yh-popover.yh-tooltip__popper {
  box-shadow: var(--yh-shadow-xl);
}
.yh-popover.yh-tooltip__popper .yh-tooltip__content {
  padding: 0;
}
