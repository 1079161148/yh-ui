...
const keyField = unref(menu?.keyField) || "key";
const labelField = unref(menu?.labelField) || "label";
</script>
...
<template>
  ...
  <yh-menu-item-group
    v-if="item.group"
    :title="item[labelField] || item.label || ''"
  >
    <yh-menu-recursive-item
      v-for="(child, idx) in item.children"
      :key="child[keyField] || child.index || idx || ''"
      :item="child"
    />
  </yh-menu-item-group>
  ...
  <yh-sub-menu
    ...
    :index="item[keyField] || item.index || ''"
