...
const titleStyle = computed(() => {
  ...
  if (menu?.mode.value === "vertical" && !menu.collapse.value) {
    ...
    const indent = isRoot ? menu.rootIndent.value ?? menu.indent.value ?? 32 : menu.indent.value ?? 32;
...
<template>
  <li
    :class="[ns.b(), ns.is('opened', isOpened), ns.is('active', isActive), ns.is('disabled', disabled), ns.is('popup', isPopup)]"
    role="menuitem"
  ...
  [ns.is('opened')]: isOpened && shouldIconRotate
}]"
          />
        </div>
...
.yh-menu {
  ...
  border-right: 1px solid var(--yh-menu-border-color);
