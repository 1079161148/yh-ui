...
import { useNamespace, useLocale, useSKU } from "@yh-ui/hooks";
  ...
  >
    <div v-if="showSelectedSummary" :class="ns.e('summary')">
      <slot name="summary" :summary="selectedSummary" :sku="selectedSku">
        ...
        <span v-else :class="ns.e('summary-placeholder')">{{ summaryPlaceholder }}</span>
      </slot>
    </div>
    ...
    <div v-for="(spec, index) in specs" :key="spec.id" :class="ns.e('item')">
      ...
      <div :class="[ns.e('values'), spec.mode ? ns.em('values', spec.mode) : '']">
        <div
          ...
          :key="value.id"
          :class="[ns.e('value'), spec.mode ? ns.em('value', spec.mode) : '', ns.is('active', isValueActive(value.id)), ns.is('disabled', !isValueSelectable(index, value.id))]"
          role="radio"
...
.yh-sku-selector {
  --yh-sku-label-color: var(--yh-text-color-secondary);
  --yh-sku-label-size: var(--yh-font-size-sm);
  --yh-sku-label-weight: var(--yh-font-weight-medium);
  --yh-sku-value-bg: var(--yh-fill-color-light);
  --yh-sku-value-color: var(--yh-text-color-primary);
  --yh-sku-value-border: var(--yh-border-color-light);
  --yh-sku-value-hover-bg: var(--yh-fill-color-dark);
  --yh-sku-value-active-bg: var(--yh-color-primary-light-9);
  --yh-sku-value-active-color: var(--yh-color-primary);
  --yh-sku-value-active-border: var(--yh-color-primary);
  --yh-sku-value-disabled-bg: var(--yh-fill-color-extra-light);
  --yh-sku-value-disabled-color: var(--yh-text-color-disabled);
  --yh-sku-value-radius: var(--yh-radius-base);
  --yh-sku-img-size: 80px;
  --yh-sku-color-swatch-size: 24px;
  --yh-sku-gap: var(--yh-spacing-md);
  display: flex;
  flex-direction: column;
  gap: var(--yh-sku-gap);
}
.yh-sku-selector.is-disabled {
  pointer-events: none;
  opacity: 0.55;
}

.yh-sku-selector__summary {
  font-size: var(--yh-font-size-sm);
  padding-bottom: 2px;
}

.yh-sku-selector__summary-text {
  color: var(--yh-text-color-primary);
  font-weight: var(--yh-font-weight-medium);
}

.yh-sku-selector__summary-placeholder {
  color: var(--yh-text-color-placeholder);
}

.yh-sku-selector__item {
  display: flex;
  flex-direction: column;
  gap: var(--yh-spacing-sm);
}

.yh-sku-selector__label {
  display: flex;
  align-items: center;
  gap: 6px;
}

.yh-sku-selector__label-name {
  font-size: var(--yh-sku-label-size);
  color: var(--yh-sku-label-color);
  font-weight: var(--yh-sku-label-weight);
}

.yh-sku-selector__values {
  display: flex;
  flex-wrap: wrap;
  gap: var(--yh-spacing-sm);
}

.yh-sku-selector__value {
  display: inline-flex;
  align-items: center;
  gap: 4px;
  padding: 6px 14px;
  border-radius: var(--yh-sku-value-radius);
  background-color: var(--yh-sku-value-bg);
  color: var(--yh-sku-value-color);
  border: 1px solid var(--yh-sku-value-border);
  cursor: pointer;
...
.yh-sku-selector__value:hover:not(.is-disabled):not(.is-active) {
  background-color: var(--yh-sku-value-hover-bg);
  border-color: var(--yh-border-color);
}
.yh-sku-selector__value.is-active {
  background-color: var(--yh-sku-value-active-bg);
  border-color: var(--yh-sku-value-active-border);
  color: var(--yh-sku-value-active-color);
  font-weight: var(--yh-font-weight-medium);
  transition: all var(--yh-transition-base);
}
.yh-sku-selector__value.is-active::after {
  content: "";
  position: absolute;
  bottom: -1px;
  right: -1px;
...
.yh-sku-selector__value.is-active::before {
  content: "";
  position: absolute;
  bottom: 3px;
  right: 2px;
...
.yh-sku-selector__value.is-disabled {
  cursor: not-allowed;
  background-color: var(--yh-sku-value-disabled-bg);
  color: var(--yh-sku-value-disabled-color);
  border: 1px dashed var(--yh-border-color-lighter);
  opacity: 0.6;
  transition: none;
}
.yh-sku-selector__value.is-disabled::after, .yh-sku-selector__value.is-disabled::before {
  display: none;
}

.yh-sku-selector__value--image {
  padding: 0;
  flex-direction: column;
  gap: 0;
  overflow: hidden;
  min-width: var(--yh-sku-img-size);
  background: transparent;
  border-radius: var(--yh-radius-base);
}

.yh-sku-selector__value--text .yh-sku-selector__value-name {
  padding: 0;
  max-width: unset;
}

.yh-sku-selector__value--color {
  padding: 3px;
  min-width: auto;
  gap: 0;
}

.yh-sku-selector__img-wrap {
  display: block;
  width: var(--yh-sku-img-size);
  height: var(--yh-sku-img-size);
  overflow: hidden;
  border-radius: calc(var(--yh-sku-value-radius) - 1px);
  background-color: var(--yh-fill-color-light);
  flex-shrink: 0;
}

.yh-sku-selector__item-img {
  width: 100%;
  height: 100%;
  object-fit: cover;
  display: block;
...
.yh-sku-selector__item-img:hover {
  transform: scale(1.05);
}

.yh-sku-selector__value-name {
  display: block;
  text-align: center;
  padding: 4px 6px;
  ...
  max-width: var(--yh-sku-img-size);
}

.yh-sku-selector__color-swatch {
  display: block;
  width: var(--yh-sku-color-swatch-size);
  height: var(--yh-sku-color-swatch-size);
  border-radius: calc(var(--yh-sku-value-radius) - 2px);
  border: 1px solid rgba(0, 0, 0, 0.08);
}

.yh-sku-selector__value-tag {
  display: inline-flex;
  align-items: center;
  padding: 0 4px;
  height: 14px;
...
.yh-sku-selector--small .yh-sku-selector__value {
  padding: 4px 10px;
  font-size: var(--yh-font-size-xs);
}

.yh-sku-selector--large .yh-sku-selector__value {
  padding: 8px 18px;
