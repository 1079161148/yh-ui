...
const buildTree = (data, parent = null, level = 0) => {
  ...
  return data.map((item) => {
    const key = item[props.nodeKey] ?? item.key ?? item.id;
    const node = {
      ...item,
      key,
...
const setCheckedNodes = (nodes) => {
  const keys = nodes.map(
    (node) => node[props.nodeKey] ?? node.key ?? node.id
  );
  setCheckedKeys(keys);
};
const setData = (data) => {
...
.yh-tree {
  --yh-tree-node-height: 28px;
  --yh-tree-text-color: var(--yh-text-color-primary, #1d1d1f);
  --yh-tree-hover-bg: var(--yh-color-primary-light-9, #ecf5ff);
  --yh-tree-current-bg: var(--yh-color-primary-light-8, #d9ecff);
  font-size: 14px;
