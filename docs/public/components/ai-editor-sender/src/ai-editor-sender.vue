...
const filterCommands = (keyword) => {
  ...
  filteredCommands.value = props.commands.filter((cmd) => {
    ...
    if (cmd.keywords?.some((k) => k.toLowerCase().includes(lowerKeyword))) return true;
    return false;
  });
};
          ...
          <div :class="ns.e('command-panel')" :style="commandPanelStyle">
            ...
            <div :class="ns.e('command-list')">
              ...
              <template v-for="(command, index) in filteredCommands" :key="command.id">
                <div
                  :class="[ns.e('command-item'), ns.is('selected', selectedCommandIndex === index), ns.is('disabled', command.disabled), ns.is('parent', !!command.children?.length)]"
                ...
                <template v-if="showCascadePanel && currentCascadeParent?.id === command.id">
                  <div
                    ...
                    :class="[ns.e('command-item'), ns.e('command-cascade-item'), ns.is('selected', selectedCommandIndex === filteredCommands.length + childIndex), ns.is('disabled', child.disabled)]"
...
.yh-ai-editor-sender {
  ...
  width: 100%;
  background-color: var(--yh-ai-editor-sender-bg);
  border: var(--yh-ai-editor-sender-border);
  border-radius: var(--yh-ai-editor-sender-radius);
  transition: all 0.3s;
  overflow: hidden;
  box-shadow: var(--yh-ai-editor-sender-shadow);
}
.yh-ai-editor-sender.is-focused {
  border-color: var(--yh-color-primary);
  box-shadow: 0 6px 20px rgba(var(--yh-color-primary-rgb), 0.15);
}

.yh-ai-editor-sender__header {
  display: flex;
  flex-wrap: wrap;
  gap: 8px;
  padding: 8px 12px;
...
.yh-ai-editor-sender__header.is-empty {
  display: none;
}

.yh-ai-editor-sender__attachment {
  display: inline-flex;
  align-items: center;
  gap: 6px;
  padding: 2px 8px;
...
.yh-ai-editor-sender__attachment:hover .yh-ai-editor-sender__attachment-remove {
  opacity: 1;
}

.yh-ai-editor-sender__attachment-icon {
  font-size: 14px;
  color: var(--yh-color-primary);
}

.yh-ai-editor-sender__attachment-remove {
  cursor: pointer;
  font-size: 12px;
  opacity: 0.6;
  transition: opacity 0.2s;
}
.yh-ai-editor-sender__attachment-remove:hover {
  color: var(--yh-color-danger);
}

.yh-ai-editor-sender__body {
  display: flex;
  flex-direction: column;
  padding: 0 12px;
}

.yh-ai-editor-sender__textarea {
  width: 100%;
  min-height: 32px;
  max-height: 300px;
  border: none;
...
.yh-ai-editor-sender__textarea::placeholder {
  color: var(--yh-text-color-placeholder);
}

.yh-ai-editor-sender__footer {
  display: flex;
  align-items: center;
...
.yh-ai-editor-sender__toolbar {
  display: flex;
  align-items: center;
  gap: 6px;
}

.yh-ai-editor-sender__actions {
  display: flex;
  align-items: center;
  gap: 8px;
}

.yh-ai-editor-sender__word-limit {
  font-size: 12px;
  color: var(--yh-text-color-placeholder);
  margin-right: 8px;
}

.yh-ai-editor-sender__command-panel-wrapper {
  position: fixed;
  z-index: 9999;
  top: 200px;
  left: 100px;
...
.yh-ai-editor-sender__command-panel {
...
.yh-ai-editor-sender__command-search {
  display: flex;
  align-items: center;
  gap: 8px;
  padding: 10px 14px;
...
.yh-ai-editor-sender__command-list {
  max-height: 280px;
  overflow: auto;
}

.yh-ai-editor-sender__command-item {
  display: flex;
  align-items: center;
  gap: 10px;
  padding: 10px 14px;
...
.yh-ai-editor-sender__command-item:hover {
  background: var(--yh-fill-color-light);
}
.yh-ai-editor-sender__command-item.is-selected {
  background: var(--yh-fill-color-light);
}
.yh-ai-editor-sender__command-item.is-disabled {
  opacity: 0.5;
  cursor: not-allowed;
}
.yh-ai-editor-sender__command-item.is-parent {
  font-weight: 500;
}

.yh-ai-editor-sender__command-cascade-item {
  padding-left: 28px;
  background: var(--yh-fill-color-extra-light);
}

.yh-ai-editor-sender__command-icon {
  font-size: 16px;
  color: var(--yh-color-primary);
}

.yh-ai-editor-sender__command-content {
  flex: 1;
  min-width: 0;
}

.yh-ai-editor-sender__command-label {
  font-size: 14px;
  font-weight: 500;
  color: var(--yh-text-color-primary);
}

.yh-ai-editor-sender__command-desc {
  display: block;
  font-size: 12px;
  color: var(--yh-text-color-secondary);
  margin-top: 2px;
...
.yh-ai-editor-sender__command-arrow {
  font-size: 14px;
  color: var(--yh-text-color-placeholder);
}

.yh-ai-editor-sender__command-empty {
  display: flex;
  flex-direction: column;
  align-items: center;
  gap: 8px;
