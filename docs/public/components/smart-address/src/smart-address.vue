...
function handleParse() {
  ...
  const provinceChildren = provinceMatch?.[props.childrenField] || [];
  ...
  const cityChildren = cityMatch?.[props.childrenField] || [];
...
const innerVal = reactive({ ...props.modelValue });
...
const cityOptions = computed(() => {
  ...
  const children = p?.[props.childrenField] || [];
  if (children.length) {
...
const districtOptions = computed(() => {
  ...
  const p = props.regionOptions.find((opt) => opt[props.valueField] === innerVal.province);
  const provinceChildren = p?.[props.childrenField] || [];
  if (provinceChildren.length) {
    const c = provinceChildren.find((child) => child[props.valueField] === innerVal.city);
    const cityChildren = c?.[props.childrenField] || [];
    if (cityChildren.length) {
...
<template>
  <div :class="[ns.b(), ns.is('disabled', disabled)]" :style="themeStyle">
    ...
    <div v-if="showParser" :class="ns.e('parser')">
      ...
      <div :class="ns.e('parser-actions')">
        <span
          v-if="parseTip"
          :class="[ns.e('parser-tip'), ns.is('success', parseStatus === 'success'), ns.is('error', parseStatus === 'error')]"
        >
          {{ parseTip }}
        </span>
        <button
  ...
  updateField('district', '');
            ...
            <YhSelect
              ...
              :placeholder="t('smartaddress.district')"
              style="flex: 1; min-width: 0"
              @update:model-value="val => updateField('district', val)"
            >
              <YhOption
      ...
      <div :class="ns.e('field')">
        ...
        <input
          ...
          @input="e => updateField('detail', e.target.value)"
        />
      </div>
