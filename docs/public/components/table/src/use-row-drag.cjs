...
const useRowDrag = options => {
  ...
  const isDraggingRow = (0, _vue.ref)(false);
  const dragRowIndex = (0, _vue.ref)(-1);
  const dropRowIndex = (0, _vue.ref)(-1);
  let lastMouseDownTarget = null;
  const isRowDragEnabled = (0, _vue.computed)(() => !!dragConfig.value?.row);
  const hasHandle = (0, _vue.computed)(() => !!dragConfig.value?.handle);
