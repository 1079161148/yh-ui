...
function useTableExport(data, columns) {
  ...
  function toXML(config = {}) {
    ...
    if (config.includeHeader !== false) {
      lines.push("  <columns>");
      if (config.showIndex) lines.push(`    <column name="${escapeXML(config.indexTitle || t("table.index"))}" />`);
      cols.forEach(col => {
  ...
  function toHTML(config = {}) {
    const cols = getExportColumns(config);
    const rows = config.data || data.value;
    const lines = ["<!DOCTYPE html>", '<html><head><meta charset="UTF-8">', "<style>", 'body{font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,sans-serif;padding:20px}', "table{width:100%;border-collapse:collapse;font-size:14px}", "th,td{padding:8px 12px;border:1px solid #dcdfe6;text-align:left}", "th{background:#f5f7fa;font-weight:600;color:#303133}", "tr:nth-child(even){background:#fafafa}", "</style></head><body>", "<table>"];
    if (config.includeHeader !== false) {
      lines.push("<thead><tr>");
      if (config.showIndex) lines.push(`<th>${escapeHTML(config.indexTitle || t("table.index"))}</th>`);
