...
const useColumnDrag = options => {
  ...
  const isDraggingColumn = (0, _vue.ref)(false);
  const dragColumnIndex = (0, _vue.ref)(-1);
  const dropColumnIndex = (0, _vue.ref)(-1);
  const isColumnDragEnabled = (0, _vue.computed)(() => !!dragConfig.value?.column);
...
exports.useColumnDrag = useColumnDrag;