...
const group = (text: string, items: Array<{ text: string; link: string }>) => ({ text, items })
const sidebar: Record<string, Array<ReturnType<typeof group>>> = {}
...
sidebar['/en/components/'] = [
  ...
  group('Config', [{ text: 'ConfigProvider', link: '/en/components/config-provider' }]),
  group('Ecommerce', [
...
sidebar['/flow/'] = [
  ...
  group('特色功能', [
    { text: '数据流光效果边', link: '/flow/data-flow-edge' },
...
export default defineConfig({
  ...
  vite: {
    ...
    ssr: { noExternal: ['monaco-editor', '@vue/repl'] }
  }
})
