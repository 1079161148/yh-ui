...
当消息数量较多时（例如超过 50 条），普通渲染方式可能会导致性能问题。AiChat 内置了虚拟滚动功能，可以高效渲染大量消息，仅渲染可见区域内的消息项，大幅提升性能。

- 自动启用：当消息数量 > 50 时，虚拟滚动将自动启用
- 手动启用：可通过 `virtual-scroll` 属性手动开启
- 可配置：支持自定义容器高度和单项预估高度
- 支持一键清空：在虚拟滚动模式下同样可以使用右上角「清空」按钮重置整个对话

<DemoBlock title="虚拟滚动示例" :ts-code="tsVirtualScroll" :js-code="jsVirtualScroll">
  <div style="height: 400px; border: 1px solid var(--yh-border-color); border-radius: 8px;">
    <yh-ai-chat
      v-model:messages="virtualMessages"
      :loading="virtualLoading"
...
::: tip 性能提示
虚拟滚动会在消息数量超过 50 条时自动启用，无需额外配置。对于需要处理成千上万条消息的聊天场景，建议开启此功能以获得流畅的滚动体验。
:::
...
| virtualScroll       | 启用虚拟滚动（消息数 > 50 时自动启用） | `boolean`                                                                            | `false` |
| virtualHeight       | 虚拟滚动容器高度                       | `number`                                                                             | `400`   |
| estimatedItemHeight | 虚拟滚动单项预估高度                   | `number`                                                                             | `80`    |
