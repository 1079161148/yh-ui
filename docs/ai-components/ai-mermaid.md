...
const longFlowchartCode = ref(`graph LR
  Start[开始流程] --> Step1[步骤 1: 收集用户需求与市场调研]
  Step1 --> Step2[步骤 2: 方案设计与技术可行性评审]
  Step2 --> Step3[步骤 3: 架构设计与核心组件模块划分]
  Step3 --> Step4[步骤 4: 前后端编码与自动化测试编写]
  Step4 --> Step5[步骤 5: 集成测试、灰度发布与生产部署]
  Step5 --> End[结束流程]`)
...
<DemoBlock title="长流程滚动与溢出" :ts-code="tsLongFlowchart" :js-code="jsLongFlowchart">
  <div style="background: var(--yh-bg-color-page); padding: 16px;">
    <yh-ai-mermaid
      header="长流程滚动示例"
