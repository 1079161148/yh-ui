...
> [!NOTE]
>
> 请注意，该功能依赖独立的截图依赖：`html-to-image`。为了保持组件库本身的轻量与纯净，我们已将它声明在组件库的可选对等依赖（`peerDependencies`）中。如果您需要使用截图或图片导出功能，请在项目中安装此依赖：
>
> ```bash
> pnpm add html-to-image # 或 npm install html-to-image
> ```
