...
除了使用 `show-minimap` 等 props 控制外，你也可以直接将小地图、控制栏和背景组件放置在 `yh-flow` 的默认插槽中，以便进行更灵活的属性配置：
...
const slotNodes = ref<Node[]>([
  { id: '1', type: 'input', position: { x: 100, y: 120 }, data: { label: '插槽放置 - 开始' }, width: 150, height: 50 },
  { id: '2', type: 'output', position: { x: 400, y: 120 }, data: { label: '插槽放置 - 结束' }, width: 150, height: 50 }
])
...
<DemoBlock title="插槽放置辅助组件" :ts-code="tsSlotDemo" :js-code="jsSlotDemo">
  <div style="border:1px solid #e2e8f0;border-radius:8px;height:500px;overflow:hidden;">
