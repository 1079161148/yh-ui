...
| type | 链接类型 | `'primary' \| 'success' \| 'warning' \| 'danger' \| 'info' \| 'secondary'` | — |
| underline | 下划线 | `boolean` | `false` |
...
| 事件名 | 说明 | 回调参数 |
| --- | --- | --- |
| `click` | `Typography.Link` 在未禁用状态下点击链接时触发 | `(event: MouseEvent)` |
...
代码和键盘样式还会额外消费全局等宽字体 token `--yh-font-family-monospace`。

### 类型导出

| 名称 | 说明 |
| --- | --- |
| `YhTypographyTitleProps` | Typography.Title 组件 Props 类型 |
| `YhTypographyTextProps` | Typography.Text 组件 Props 类型 |
| `YhTypographyParagraphProps` | Typography.Paragraph 组件 Props 类型 |
| `YhTypographyLinkProps` | Typography.Link 组件 Props 类型 |
| `YhTypographySlots` | Typography 通用插槽类型 |
| `YhTypographyTitleLevel` | 标题层级联合类型 |
| `YhTypographyType` | 文本类型联合类型 |
| `YhTypographyTitleInstance` | Typography.Title 实例类型 |
| `YhTypographyTextInstance` | Typography.Text 实例类型 |
| `YhTypographyParagraphInstance` | Typography.Paragraph 实例类型 |
| `YhTypographyLinkInstance` | Typography.Link 实例类型 |
