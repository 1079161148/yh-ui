...
| theme-overrides       | 组件级主题覆盖变量，用于定制 Slider 样式                | `ComponentThemeVars`                | —           |
...
| 名称 | 说明 | 类型 |
| --- | --- | --- |
| `sliderRef` | 滑块根元素引用。 | `Ref<HTMLElement \| undefined>` |
| `firstValue` | 当前主滑块值。 | `Ref<number>` |
| `secondValue` | 范围模式下的第二个滑块值。 | `Ref<number>` |
...
| 名称 | 说明 |
| --- | --- |
| `YhSliderProps` | 组件 Props 类型 |
| `YhSliderEmits` | 组件事件类型 |
| `YhSliderSlots` | 组件插槽类型 |
| `YhSliderExpose` | 组件 Expose 类型 |
| `YhSliderInstance` | 组件实例类型 |
