...
接入 `@yh-ui/nuxt` 后，可直接在页面与组件中使用 `YhDrawer`。抽屉外壳、标题、默认内容和页脚都可以参与 SSR 渲染；遮罩、焦点控制与挂载到 `body` 的面板交互会在客户端 hydration 后继续工作。
...
| 名称 | 说明 |
| --- | --- |
| `YhDrawerProps` | `YhDrawer` props 类型 |
| `YhDrawerEmits` | `YhDrawer` emits 类型 |
| `YhDrawerSlots` | `YhDrawer` slots 类型 |
| `YhDrawerExpose` | `YhDrawer` expose 类型 |
| `YhDrawerInstance` | `YhDrawer` 实例类型 |
