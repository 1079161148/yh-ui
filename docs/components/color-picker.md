...
| 名称 | 说明 |
| --- | --- |
| `YhColorPickerProps` | `YhColorPicker` props 类型 |
| `YhColorPickerEmits` | `YhColorPicker` emits 类型 |
| `YhColorPickerSlots` | `YhColorPicker` slots 类型 |
| `YhColorPickerExpose` | `YhColorPicker` expose 类型 |
| `YhColorPickerInstance` | `YhColorPicker` 实例类型 |
...
| 名称 | 说明 | 类型 |
| --- | --- | --- |
| `visible` | 当前面板显示状态。 | `Ref<boolean>` |
| `togglePopper` | 切换面板展开状态。 | `() => void` |
| `handleClear` | 清空当前颜色值。 | `() => void` |
| `handleConfirm` | 确认当前颜色值。 | `() => void` |
