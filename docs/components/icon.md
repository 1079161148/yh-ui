...
| icon   | 自定义图标组件或已注册图标名称      | `Component \| string` | —              |
...
`YhIcon` 支持 `themeOverrides`，并主要消费全局主题令牌，如文本色、主色、危险色和尺寸相关令牌。当前组件未定义额外的专属组件级 CSS 变量。

### 类型导出

| 名称             | 说明                |
| ---------------- | ------------------- |
| `YhIconProps`    | `YhIcon` props 类型 |
| `YhIconSlots`    | `YhIcon` slots 类型 |
| `YhIconData`     | 图标数据结构类型    |
| `YhIconSet`      | 内置图标集类型      |
| `YhIconInstance` | `YhIcon` 实例类型   |
