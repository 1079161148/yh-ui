...
const tsBasic = `<template>
  <yh-waterfall :items="list" :cols="3" :gap="16" height-property="height">
    <template #default="{ item }">
      <div class="waterfall-demo-card" :class="'card-' + item.id" :style="{
        ...
        <p style="margin: 8px 0 0; font-size: 12px; color: var(--yh-text-color-secondary);">{{ item.desc }}</p>
      </div>
    </template>
  </yh-waterfall>
</template>
...
const tsStates = `<template>
  ...
  <yh-waterfall :items="list" :loading="loading" :cols="3" :gap="16">
    <template #default="{ item }">
      <div class="waterfall-demo-card" :class="'card-' + item.id" style="display: flex; align-items: center; justify-content: center; border-radius: 12px; font-size: 13px; border: 1px solid var(--yh-border-color-light);" :style="{ height: item.height + 'px', background: item.color }">
        {{ item.title }}
      </div>
    </template>
    <template #empty>
      <div style="padding: 60px 0; text-align: center; color: var(--yh-text-color-secondary);">
...
const tsNuxt = `<template>
  <yh-waterfall :items="list" :cols="3" :gap="16">
    <template #default="{ item }">
      <div class="waterfall-demo-card" :class="'card-' + item.id" style="display: flex; align-items: center; justify-content: center; border-radius: 8px; border: 1px solid var(--yh-border-color-light);" :style="{ background: item.color, height: (item.height / 2) + 'px' }">
        {{ item.title }}
      </div>
    </template>
  </yh-waterfall>
...
.custom-loading { color: var(--yh-color-primary); font-weight: bold; background: var(--yh-bg-color-overlay); box-shadow: var(--yh-shadow-md); padding: 8px 16px; border-radius: 20px; border: 1px solid var(--yh-color-primary-light-5); }
.waterfall-demo-card {
  color: var(--yh-text-color-primary);
  transition: background-color 0.3s, color 0.3s;
}
html.dark .waterfall-demo-card.card-1 { background-color: rgba(64, 158, 255, 0.15) !important; }
html.dark .waterfall-demo-card.card-2 { background-color: rgba(245, 108, 108, 0.15) !important; }
html.dark .waterfall-demo-card.card-3 { background-color: rgba(103, 194, 58, 0.15) !important; }
html.dark .waterfall-demo-card.card-4 { background-color: rgba(230, 162, 60, 0.15) !important; }
html.dark .waterfall-demo-card.card-5 { background-color: rgba(186, 85, 211, 0.15) !important; }
html.dark .waterfall-demo-card.card-6 { background-color: rgba(255, 105, 180, 0.15) !important; }
</style>
